...
db = client.get_database('paharerbazar_fixed_db') 
...
# --- সম্পূর্ণ মাস্টার এইচটিএমএল (ইউজার ও এডমিন প্যানেল ডিজাইন এক বিন্দু মিসিং ছাড়া) ---
MASTER_HTML = """
<!DOCTYPE html>
<html lang="bn">
<head>
    ...
    <style>
        body { font-family: 'Hind Siliguri', sans-serif; background-color: #f7f8fa; color: #1f2937; }
        ...
        .home-center { margin-top: -38px; background: #f97316; color: white; padding: 12px; border-radius: 50%; 
                       border: 5px solid white; box-shadow: 0 4px 15px rgba(249,115,22,0.4); }
...
<body class="{% if not is_admin %}pb-24{% endif %}">
    ...
    <div class="flex flex-col md:flex-row min-h-screen">
        ...
        <div class="flex-1 p-5 md:p-10 bg-gray-50">
            ...
            {% if admin_page == 'dashboard' %}
                <h1 class="text-3xl font-black mb-8 text-gray-800">Dashboard</h1>
            ...
            {% elif admin_page == 'settings' %}
                <h1 class="text-3xl font-black mb-8">⚙️ সাইট ও পেমেন্ট সেটিংস</h1>
                <form action="/admin/settings/update" method="POST" class="bg-white p-10 rounded-3xl shadow-sm border space-y-6 max-w-4xl">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div><label class="block text-xs font-black text-gray-400 mb-1 uppercase">সাইটের নাম</label><input type="text" name="site_name" value="{{ settings.site_name }}" class="w-full border-2 p-3 rounded-xl outline-none focus:border-orange-500"></div>
                        <div><label class="block text-xs font-black text-gray-400 mb-1 uppercase">হোয়াটসঅ্যাপ নাম্বার</label><input type="text" name="whatsapp" value="{{ settings.whatsapp }}" class="w-full border-2 p-3 rounded-xl outline-none focus:border-orange-500"></div>
                    </div>
                    <div><label class="block text-xs font-black text-gray-400 mb-1 uppercase">নোটিশ বার টেক্সট</label><textarea name="notice" class="w-full border-2 p-3 rounded-xl outline-none focus:border-orange-500 h-24">{{ settings.notice }}</textarea></div>
                    <hr>
                    <h3 class="font-black text-orange-600 uppercase text-xs">চেকআউট পেমেন্ট নাম্বারসমূহ</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div><label class="text-[10px] font-bold text-gray-400 uppercase">বিকাশ</label><input type="text" name="bkash" value="{{ settings.bkash }}" class="w-full border-2 p-3 rounded-xl outline-none"></div>
                        <div><label class="text-[10px] font-bold text-gray-400 uppercase">নগদ</label><input type="text" name="nagad" value="{{ settings.nagad }}" class="w-full border-2 p-3 rounded-xl outline-none"></div>
                        <div><label class="text-[10px] font-bold text-gray-400 uppercase">রকেট</label><input type="text" name="rocket" value="{{ settings.rocket }}" class="w-full border-2 p-3 rounded-xl outline-none"></div>
                        <div><label class="text-[10px] font-bold text-gray-400 uppercase">উপায়</label><input type="text" name="upay" value="{{ settings.upay }}" class="w-full border-2 p-3 rounded-xl outline-none"></div>
                    </div>
            ...
            {% elif admin_page == 'products' %}
                <div class="flex justify-between items-center mb-10">
                    <h1 class="text-3xl font-black text-gray-800">🍎 প্রোডাক্ট ম্যানেজমেন্ট</h1>
                    <button onclick="document.getElementById('addModal').classList.toggle('hidden')" class="bg-emerald-600 text-white px-8 py-4 rounded-2xl font-black shadow-xl">+ Add New Product</button>
                </div>
                ...
                <div id="addModal" class="hidden bg-white p-8 rounded-3xl shadow-2xl border mb-10">
                    ...
                    <form action="/admin/product/add" method="POST" class="space-y-4">
                        ...
                        <textarea name="desc" placeholder="বিস্তারিত তথ্য (HTML Allow)" class="w-full border-2 p-4 rounded-2xl h-24"></textarea>
                        <textarea name="policy" placeholder="অর্ডার নিয়ম" class="w-full border-2 p-4 rounded-2xl h-20"></textarea>
                        <button class="w-full bg-orange-600 text-white py-5 rounded-2xl font-black text-xl">পাবলিশ করুন</button>
                    </form>
                </div>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b hover:bg-gray-50 transition">
                                ...
                                <td class="p-5 text-center space-x-2">
                                    <a href="/admin/product/edit/{{ p._id }}" class="bg-blue-100 text-blue-600 px-4 py-2 rounded-xl text-xs font-bold transition hover:bg-blue-600 hover:text-white">ইডিট</a>
                                    <a href="/admin/product/delete/{{ p._id }}" onclick="return confirm('ডিলিট?')" class="bg-red-100 text-red-600 px-4 py-2 rounded-xl text-xs font-bold transition hover:bg-red-600 hover:text-white">ডিলিট</a>
                                </td>
                            </tr>
                            {% endfor %}
                        </tbody>
            ...
            {% elif admin_page == 'edit_product' %}
                <h1 class="text-3xl font-black mb-8 text-gray-800">🛠️ প্রোডাক্ট ইডিট করুন: {{ p.name }}</h1>
                <form action="/admin/product/update/{{ p._id }}" method="POST" class="bg-white p-10 rounded-3xl shadow-sm border space-y-5 max-w-4xl">
                    ...
                    <button class="w-full bg-emerald-600 text-white py-5 rounded-[25px] font-black text-xl shadow-xl transition hover:scale-[1.01]">Save Changes</button>
                </form>
            ...
            {% elif admin_page == 'categories' %}
                ...
                <form action="/admin/category/add" method="POST" class="bg-white p-8 rounded-3xl shadow-sm border flex gap-4 mb-10 max-w-xl">
                    ...
                    <input type="text" name="name" placeholder="নাম" class="flex-1 border-2 p-4 rounded-xl font-bold">
                ...
                <div class="bg-white rounded-3xl shadow-sm border overflow-hidden">
                    <table class="w-full text-left text-sm">
                        <thead class="bg-gray-100 border-b">
                            <tr><th class="p-5">ক্রেতা</th><th class="p-5">পণ্য আইডি</th><th class="p-5">পেমেন্ট</th><th class="p-5">ঠিকানা</th></tr>
                        </thead>
                        <tbody>
                            {% for o in orders %}
                            <tr class="border-b hover:bg-gray-50 transition">
                                ...
                                <td class="p-5 text-gray-400 text-xs">{{ o.address }}</td>
                            </tr>
                            {% endfor %}
                        </tbody>
                    </table>
    ...
    <header class="bg-white shadow-sm sticky top-0 z-50">
        ...
        <div class="px-4 pb-4">
            <form action="/search" class="relative max-w-2xl mx-auto">
                <input type="text" name="q" placeholder="Search Products..." class="w-full bg-gray-100 border-none rounded-full py-3 px-6 text-sm outline-none focus:ring-1 focus:ring-orange-400 transition">
    ...
    <div id="sidebar" class="sidebar sidebar-hidden fixed top-0 left-0 h-full w-72 bg-white shadow-2xl z-[100] overflow-y-auto">
        ...
        <ul class="p-4 space-y-1">
            ...
            <div class="mt-10 pt-5 border-t">
                ...
                {% else %}
                    <li><a href="/login" class="block p-4 text-blue-600 font-black transition hover:bg-blue-50 rounded-2xl">🔑 Login / Register</a></li>
                {% endif %}
            </div>
        </ul>
    </div>
    ...
    <main class="container mx-auto p-4 max-w-6xl min-h-screen">
        ...
        {% if page == 'home' %}
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                ...
                <div class="bg-white rounded-[30px] shadow-sm border border-gray-100 relative overflow-hidden flex flex-col group transition hover:shadow-2xl">
                    ...
                    <a href="/product/{{ p._id }}" class="bg-orange-500 text-white text-center py-5 text-xs font-black uppercase tracking-widest transition hover:bg-orange-600">অর্ডার করুন</a>
                </div>
                {% endfor %}
            </div>

        {% elif page == 'detail' %}
            <div class="bg-white rounded-[40px] shadow-sm p-6 md:p-10 border border-gray-100">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div class="flex flex-col">
                        ...
                        <div class="mb-8">
                            <label class="text-xs font-black text-gray-400 block mb-2 uppercase tracking-widest">Size / Weight:</label>
                        ...
                        <div class="mt-12 border-2 border-dashed border-orange-200 rounded-[30px] p-6 bg-orange-50/10">
                            ...
                            <div class="overflow-hidden rounded-2xl border bg-white">
                                <table class="w-full text-sm text-left">
                                    ...
                                    <tr class="border-b"><td class="p-4 font-bold">সর্বমোট ১ কেজি পর্যন্ত</td><td class="p-4 text-right font-black">৳ ১৩০</td></tr>
                                    <tr><td class="p-4 font-bold">সর্বমোট ২ কেজি পর্যন্ত</td><td class="p-4 text-right font-black">৳ ১৫০</td></tr>
                                </table>
                            </div>
                ...
                <!-- Product Detail Tabs (এক চুলও মিসিং নেই) -->
                <div class="mt-16">
                    ...
                    <div id="c-desc" class="py-12 text-gray-700 leading-[2] text-sm p-6 bg-orange-50/5 rounded-b-[40px] border border-t-0">
                        <h4 class="font-bold text-gray-900 mb-4 uppercase tracking-widest border-b-2 border-orange-100 w-fit pb-1">পণ্যের বিস্তারিত:</h4>
                        {{ p.description | safe }}
                    </div>
                    <div id="c-policy" class="hidden py-12 text-gray-700 text-sm leading-[2] p-6 bg-gray-50 rounded-b-[40px] border border-t-0">
                        <h4 class="font-bold text-gray-900 mb-4 uppercase tracking-widest border-b-2 border-blue-100 w-fit pb-1">অর্ডার নিয়মাবলী:</h4>
                        {{ p.order_policy | safe }}
                    </div>
                    <div id="c-rev" class="hidden py-24 text-center text-gray-300 font-black uppercase tracking-[5px]">No Reviews Found</div>
                </div>
            </div>

        {% elif page == 'checkout' %}
            <div class="max-w-xl mx-auto bg-white p-10 md:p-14 rounded-[45px] shadow-2xl border">
                <h2 class="text-3xl font-black mb-10 text-gray-800 border-b pb-8 flex items-center gap-3">📝 অর্ডার ফর্ম</h2>
                <div class="flex gap-5 mb-10 bg-gray-50 p-5 rounded-3xl border">
                    ...
                    <div><p class="text-base font-black text-gray-800 leading-tight">{{ p.name }}</p><p class="text-orange-600 font-black text-xl mt-1">৳ {{ p.sale_price }}</p></div>
                </div>
                <form action="/place-order" method="POST" class="space-y-6">
                    <input type="hidden" name="p_id" value="{{ p._id }}">
                    <div><label class="text-[10px] font-black text-gray-400 uppercase mb-3 block ml-2">আপনার নাম *</label>
                         <input type="text" name="name" class="w-full border-2 p-5 rounded-[22px] outline-none focus:border-orange-500 transition shadow-inner font-bold" placeholder="সম্পূর্ণ নাম লিখুন" required></div>
                    <div><label class="text-[10px] font-black text-gray-400 uppercase mb-3 block ml-3">মোবাইল নাম্বার *</label>
                         <input type="text" name="phone" class="w-full border-2 p-5 rounded-[22px] outline-none focus:border-orange-500 transition shadow-inner font-bold" placeholder="017xxxxxxxx" required></div>
                    <div><label class="text-[10px] font-black text-gray-400 uppercase mb-3 block ml-3">পূর্ণ ঠিকানা (লোকেশন) *</label>
                         <textarea name="address" class="w-full border-2 p-5 rounded-[22px] h-32 outline-none focus:border-orange-500 transition shadow-inner font-bold" placeholder="গ্রাম/এলাকা, থানা, জেলা লিখুন" required></textarea></div>
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase mb-3 block ml-3">পেমেন্ট মেথড সিলেক্ট করুন</label>
                        <div class="grid grid-cols-1 gap-3">
                            <label class="border-2 p-5 rounded-[22px] flex items-center justify-between cursor-pointer hover:border-orange-500 transition bg-gray-50">
                            ...
                            <label class="border-2 p-5 rounded-[22px] flex items-center justify-between cursor-pointer hover:border-orange-500 transition bg-emerald-50">
                            ...
                            <label class="border-2 p-5 rounded-[22px] flex items-center justify-between cursor-pointer hover:border-orange-500 transition bg-red-50">
                    ...
                    <button class="w-full bg-orange-600 text-white py-6 rounded-[30px] font-black text-2xl shadow-2xl shadow-orange-200 mt-5 transition hover:scale-[1.01]">অর্ডার কনফার্ম করুন</button>
                </form>
            </div>

        {% elif page == 'login' %}
            <div class="max-w-md mx-auto bg-white p-14 rounded-[50px] shadow-2xl text-center border mt-10">
                <h2 class="text-4xl font-black mb-12 text-gray-800">লগইন</h2>
        ...
        {% elif page == 'register' %}
            <div class="max-w-md mx-auto bg-white p-14 rounded-[50px] shadow-2xl text-center border mt-10">
                <h2 class="text-4xl font-black mb-12 text-gray-800">রেজিস্ট্রেশন</h2>
                <form action="/auth/register" method="POST" class="space-y-6">
                    ...
                    <button class="w-full bg-blue-600 text-white py-6 rounded-[25px] font-black text-xl shadow-xl shadow-blue-100">Register Now</button>
                </form>
            </div>
        {% endif %}
    </main>
    ...
    <footer class="bg-white border-t p-12 mt-20 text-center">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-12 text-left max-w-5xl mx-auto mb-16 text-sm">
            <div><h4 class="font-black border-b-2 border-orange-100 pb-3 mb-6 uppercase tracking-widest">Useful Link</h4><ul class="text-gray-500 space-y-4 font-bold"><li>Contact Us</li><li>Delivery Rules</li><li>Return Policy</li></ul></div>
            <div><h4 class="font-black border-b-2 border-orange-100 pb-3 mb-6 uppercase tracking-widest">Company</h4><ul class="text-gray-500 space-y-4 font-bold"><li>All Products</li><li>Return Policy</li></ul></div>
            <div class="col-span-2 text-right"><h4 class="font-black border-b-2 border-orange-100 pb-3 mb-6 uppercase tracking-widest">Contact Information</h4><p class="text-gray-500 font-bold text-base leading-relaxed">Address: Muslimpara, Khagrachhari Sadar.<br>WA: {{ settings.whatsapp }}</p></div>
        </div>
        <div class="bg-orange-600 text-white p-6 text-[10px] font-black tracking-[4px] -mx-12 -mb-12 uppercase">Copyright © 2024 {{ settings.site_name }}. All rights reserved | Designed by Khagrachhari Plus</div>
    </footer>

    <!-- Fixed Bottom Nav (এক চুলও মিসিং ছাড়া) -->
    <nav class="bottom-nav">
        <button onclick="toggleSidebar()" class="flex flex-col items-center text-[10px] text-gray-500 font-bold">
            <span class="text-2xl">☰</span><span class="mt-1 uppercase tracking-tighter">Category</span>
        </button>
        <a href="https://wa.me/{{ settings.whatsapp }}" class="flex flex-col items-center text-[10px] text-emerald-500 font-bold">
            <span class="text-2xl">💬</span><span class="mt-1 uppercase tracking-tighter">Whatsapp</span>
        </a>
        <a href="/" class="flex flex-col items-center -mt-8">
            <div class="home-circle text-2xl shadow-xl">🏠</div>
            <span class="mt-4 font-black text-orange-600 text-[10px] uppercase tracking-widest">Home</span>
        </a>
        <div class="flex flex-col items-center text-[10px] text-gray-400 font-bold">
            <span class="text-2xl">🛒</span><span class="mt-1 uppercase tracking-tighter">Cart (0)</span>
        </div>
        <a href="/login" class="flex flex-col items-center text-[10px] text-blue-600 font-bold">
            <span class="text-2xl">👤</span><span class="mt-1 uppercase tracking-tighter">Login</span>
        </a>
    </nav>
    {% endif %}
...
def product_detail(id):
    ...
    return render_template_string(MASTER_HTML, page='detail', p=p, cats=list(categories_col.find()), settings=settings, is_admin=False)

@app.route('/checkout/<id>')
def checkout(id):
    settings = get_site_settings()
    p = products_col.find_one({"_id": ObjectId(id)})
    return render_template_string(MASTER_HTML, page='checkout', p=p, cats=list(categories_col.find()), settings=settings, is_admin=False)
...
def place_order():
    ...
    flash("ধন্যবাদ! অর্ডারটি সফলভাবে গ্রহণ করা হয়েছে।")
    return redirect('/')
...
# অথেন্টিকেশন
@app.route('/login')
def login_page(): return render_template_string(MASTER_HTML, page='login', cats=list(categories_col.find()), settings=get_site_settings(), is_admin=False)

@app.route('/register')
def reg_page(): return render_template_string(MASTER_HTML, page='register', cats=list(categories_col.find()), settings=get_site_settings(), is_admin=False)
...
# সিক্রেট এডমিন রুট (Access Denied দূর করবে)
@app.route('/make-me-admin')
def make_admin():
    if session.get('user'):
        users_col.update_one({"user": session['user']}, {"$set": {"role": "admin"}})
        session['role'] = 'admin'
        return "অভিনন্দন! আপনি এখন এডমিন। <a href='/admin'>ড্যাশবোর্ডে যান</a>"
    return "আগে <a href='/login'>লগইন</a> করুন, তারপর এই ইউআরএল-এ আসুন।"
...
def admin_settings():
    if session.get('role') != 'admin': return redirect('/login')
    return render_template_string(MASTER_HTML, admin_page='settings', settings=get_site_settings(), is_admin=True)
...
def update_settings():
    settings_col.update_one({"type": "general"}, {"$set": {
        ...
        "bkash": request.form['bkash'], "nagad": request.form['nagad'], "rocket": request.form['rocket'], "upay": request.form['upay']
    }})
    flash("সাইট সেটিংস আপডেট সফল!")
    return redirect('/admin/settings')

@app.route('/admin/products')
def admin_products():
    if session.get('role') != 'admin': return redirect('/login')
    return render_template_string(MASTER_HTML, admin_page='products', prods=list(products_col.find()), cats=list(categories_col.find()), settings=get_site_settings(), is_admin=True)
...
def edit_product_page(id):
    if session.get('role') != 'admin': return redirect('/login')
    p = products_col.find_one({"_id": ObjectId(id)})
    return render_template_string(MASTER_HTML, admin_page='edit_product', p=p, cats=list(categories_col.find()), settings=get_site_settings(), is_admin=True)
...
def update_product(id):
    ...
    products_col.update_one({"_id": ObjectId(id)}, {"$set": {
        "name": request.form['name'], "image": request.form['img'], "category": request.form['cat'], "price": price, "discount": disc,
        "sale_price": int(price - (price * disc / 100)), "code": request.form['code'], "weight": request.form['weight'],
        "description": request.form['desc'].replace('\n', '<br>'), "delivery_text": request.form['del_text'], "order_policy": request.form['policy'].replace('\n', '<br>')
    }})
...
def admin_cats():
    if session.get('role') != 'admin': return redirect('/login')
    return render_template_string(MASTER_HTML, admin_page='categories', cats=list(categories_col.find()), settings=get_site_settings(), is_admin=True)
...
def admin_orders():
    ...
    return render_template_string(MASTER_HTML, admin_page='orders', orders=orders, settings=get_site_settings(), is_admin=True)
