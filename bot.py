...
app = Flask(__name__)
app.secret_key = "paharer_bazar_absolute_ultimate_mega_final_2024_fixed"

# --- মংগোডিবি কানেকশন (আপনার দেওয়া অরিজিনাল লিঙ্ক) ---
MONGO_URI = "mongodb+srv://Demo270:Demo270@cluster0.ls1igsg.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
client = MongoClient(MONGO_URI)
db = client.get_database('paharer_bazar_fixed') # নির্দিষ্ট নাম দেওয়া হলো যাতে ১০০ ডাটাবেস এরর না আসে
...
# প্রাথমিক সেটিংস লোডার (যাতে Internal Server Error না আসে)
...
# --- সম্পূর্ণ মাস্টার এইচটিএমএল (ইউজার ও এডমিন প্যানেল এক বিন্দু মিসিং ছাড়া) ---
MASTER_HTML = """
<!DOCTYPE html>
<html lang="bn">
<head>
    ...
    <style>
        ...
        .discount-badge {
            ...
            top: 10px; right: 10px; z-index: 30; line-height: 1.1;
...
<body class="{% if not is_admin %}pb-24{% endif %}">
    ...
    <div class="flex flex-col md:flex-row min-h-screen">
        <div class="w-full md:w-64 admin-sidebar p-6 flex flex-col">
            <h2 class="text-2xl font-black text-orange-500 border-b border-gray-700 pb-4 mb-8">Admin Panel</h2>
            <nav class="flex-grow space-y-2">
                <a href="/admin" class="block p-3 rounded-xl hover:bg-gray-800 transition">📊 ড্যাশবোর্ড</a>
                <a href="/admin/settings" class="block p-3 rounded-xl hover:bg-gray-800 transition">⚙️ সাইট ও পেমেন্ট সেটিংস</a>
                <a href="/admin/products" class="block p-3 rounded-xl hover:bg-gray-800 transition">🍎 প্রোডাক্ট ম্যানেজমেন্ট</a>
                <a href="/admin/categories" class="block p-3 rounded-xl hover:bg-gray-800 transition">📂 ক্যাটাগরি ম্যানেজমেন্ট</a>
                <a href="/admin/orders" class="block p-3 rounded-xl hover:bg-gray-800 transition">📦 অর্ডার তালিকা</a>
            </nav>
        ...
        <div class="flex-1 p-5 md:p-10 bg-gray-50">
            ...
            {% elif admin_page == 'settings' %}
                ...
                <form action="/admin/settings/update" method="POST" class="bg-white p-10 rounded-[35px] shadow-sm border space-y-6 max-w-4xl">
                    ...
                    <hr>
                    <h3 class="font-black text-orange-600 uppercase text-sm">পেমেন্ট মেথড নাম্বার চেঞ্জ</h3>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div><label class="text-[10px] font-bold text-gray-400 uppercase">রকেট/উপায়</label><input type="text" name="rocket" value="{{ settings.rocket }}" class="w-full border-2 p-3 rounded-xl outline-none"></div>
                    </div>
                    <button class="w-full bg-orange-600 text-white py-5 rounded-[25px] font-black text-xl shadow-xl mt-5">Save All Changes</button>
                </form>
            ...
            {% elif admin_page == 'products' %}
                ...
                <div id="addModal" class="hidden bg-white p-8 rounded-[35px] shadow-2xl border mb-10">
                    <h2 class="text-xl font-black mb-6 border-b pb-3">নতুন প্রোডাক্ট যোগ করুন</h2>
                    <form action="/admin/product/add" method="POST" class="space-y-4 text-sm">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <input type="text" name="name" placeholder="প্রোডাক্ট নাম" class="border-2 p-4 rounded-2xl w-full" required>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <input type="text" name="weight" placeholder="ওজন (৫০০ গ্রাম)" class="border-2 p-4 rounded-2xl">
                        </div>
                        ...
                        <button class="w-full bg-orange-600 text-white py-5 rounded-[25px] font-black text-xl">পণ্যটি পাবলিশ করুন</button>
                    </form>
                </div>
                ...
                <div class="bg-white rounded-[35px] shadow-sm border overflow-hidden">
                    <table class="w-full text-left">
                        <thead class="bg-gray-50 border-b">
                            <tr><th class="p-5">Image</th><th class="p-5">Product Info</th><th class="p-5">Price</th><th class="p-5 text-center">Action</th></tr>
                        </thead>
                        <tbody>
                            {% for p in prods %}
                            <tr class="border-b hover:bg-gray-50 transition">
                                ...
                                <td class="p-5 text-center space-x-2">
                                    <a href="/admin/product/edit/{{ p._id }}" class="bg-blue-100 text-blue-600 px-4 py-2 rounded-xl text-xs font-bold">ইডিট</a>
                                    <a href="/admin/product/delete/{{ p._id }}" onclick="return confirm('ডিলিট করতে চান?')" class="bg-red-100 text-red-600 px-4 py-2 rounded-xl text-xs font-bold">ডিলিট</a>
                                </td>
                            </tr>
                            {% endfor %}
                        </tbody>
            ...
            {% elif admin_page == 'edit_product' %}
                <h1 class="text-3xl font-black mb-8 text-gray-800">🛠️ প্রোডাক্ট ইডিট করুন</h1>
    ...
    <main class="container mx-auto p-4 max-w-6xl min-h-screen">
        ...
        {% if page == 'home' %}
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                ...
                <div class="bg-white rounded-[30px] shadow-sm border border-gray-100 relative overflow-hidden flex flex-col group transition hover:shadow-2xl">
                    {% if p.discount > 0 %}<div class="discount-badge">{{ p.discount }}%<br>ছাড়</div>{% endif %}
                    ...
                    <a href="/product/{{ p._id }}" class="bg-orange-500 text-white text-center py-5 text-xs font-black uppercase tracking-widest transition">অর্ডার করুন</a>
                </div>
                {% endfor %}
            </div>
            ...
            <div class="bg-white rounded-[40px] shadow-sm p-6 md:p-10 border border-gray-100">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div class="flex flex-col">
                        ...
                        <div class="mb-10">
                            <label class="text-xs font-black text-gray-400 block mb-2 uppercase tracking-widest">Weight:</label>
                        ...
                        <div class="mt-12 border-2 border-dashed border-orange-200 rounded-[30px] p-6 bg-orange-50/10">
                            ...
                            <div class="overflow-hidden rounded-2xl border bg-white text-sm">
                                <table class="w-full text-left">
        ...
        {% elif page == 'checkout' %}
            <div class="max-w-xl mx-auto bg-white p-10 md:p-12 rounded-[45px] shadow-2xl border border-gray-50">
                ...
                <form action="/place-order" method="POST" class="space-y-6">
                    ...
                    <input type="text" name="name" class="w-full border-2 p-5 rounded-[22px] font-bold outline-none focus:border-orange-500" placeholder="সম্পূর্ণ নাম লিখুন" required>
                    ...
                    <textarea name="address" class="w-full border-2 p-5 rounded-[22px] h-32 font-bold outline-none focus:border-orange-500" placeholder="বিস্তারিত ঠিকানা (গ্রাম, থানা, জেলা)" required></textarea>
                    <div>
                        ...
                        <select name="payment" class="w-full border-2 p-5 rounded-[22px] bg-white font-bold outline-none">
                            ...
                            <option value="Bkash">বিকাশ (Send to: {{ settings.bkash }})</option>
                            <option value="Nagad">নগদ (Send to: {{ settings.nagad }})</option>
                        </select>
                    </div>
                    <button class="w-full bg-orange-600 text-white py-6 rounded-[28px] font-black text-2xl shadow-2xl transition hover:scale-[1.01]">অর্ডার কনফার্ম করুন</button>
                </form>
            </div>
    ...
    <footer class="bg-white border-t p-12 mt-20 text-center">
        ...
        <div class="bg-orange-600 text-white p-8 text-[10px] font-black tracking-[4px] -mx-12 -mb-12 uppercase">Copyright © 2024 {{ settings.site_name }}. All rights reserved.</div>
    </footer>
...
# --- এডমিন রাউটস (ফুল ফিচার এক বিন্দু মিসিং ছাড়া) ---
...
def admin_products():
    ...
    prods = list(products_col.find())
    cats = list(categories_col.find())
    return render_template_string(MASTER_HTML, admin_page='products', prods=prods, cats=cats, settings=get_site_settings(), is_admin=True)
